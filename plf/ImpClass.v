...
Definition cfg : Type := com * state.

Inductive ceval_small : cfg -> cfg -> Prop :=
  | ES_Skip : forall c st,
    ceval_small (<{skip; c}>, st) (c, st)
  | ES_Assgn : forall X a n c st,
    aeval st a = n ->
    ceval_small (<{X := a; c}>, st) (c, (X !-> n; st))
  | ES_Seq : forall c0 c1 c st,
    ceval_small (<{(c0; c1); c}>, st) (<{c0; c1; c}>, st)
  | ES_IfTrue : forall b c0 c1 c st,
    beval st b = true ->
    ceval_small (<{if b then c0 else c1 end; c}>, st) (<{c0; c}>, st)
  | ES_IfFalse : forall b c0 c1 c st,
    beval st b = false ->
    ceval_small (<{if b then c0 else c1 end; c}>, st) (<{c1; c}>, st)
  | ES_WhileTrue : forall b cb c st,
    beval st b = true ->
    ceval_small (<{while b do cb end; c}>, st) (<{cb; while b do cb end; c}>, st)
